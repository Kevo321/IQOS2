package com.digitas.bubblepicker.physics

import com.digitas.bubblepicker.rendering.Item
import com.digitas.bubblepicker.sqr
...
object Engine {
    ...
    fun build(bodiesCount: Int, radiuses: Array<Int>?, scaleX: Float, scaleY: Float): List<CircleBody> {
        val density = interpolate(0.8f, 0.2f, radius / 100f)
        for (i in 0..bodiesCount - 1) {
            val customRadius = if (radiuses != null) {
                interpolate(0.1f, 0.25f, radiuses[i] / 100f)
            } else {
                bubbleRadius
            }
